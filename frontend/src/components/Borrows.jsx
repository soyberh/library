...
function Borrows() {
  ...
  const fetchData = async () => {
    try {
      const [borrowsRes, booksRes, membersRes] = await Promise.all([
        api.get("/borrows"), // Updated URL path
        api.get("/books"),   // Updated URL path
        api.get("/members")  // Updated URL path
      ]);
  ...
  const handleSave = () => {
    ...
    if (editId) {
      api.put(`/borrows/${editId}`, borrowData).then(() => { // Updated
        clearForm();
        fetchData();
      });
    } else {
      api.post("/borrows", borrowData).then(() => { // Updated
        clearForm();
        fetchData();
      });
    }
  ...
  const deleteBorrow = (id) => {
    if (window.confirm("Are you sure?")) {
      api.delete(`/borrows/${id}`).then(fetchData); // Updated
    }
  };
    ...
    <div className="p-6">
      ...
      <div className="flex justify-center mb-6">
        <div className="bg-white p-4 shadow w-full max-w-md rounded border-t-4 border-blue-400">
          ...
          <select className="border p-2 w-full mb-2 bg-white" value={member_id} onChange={e => setMemberId(e.target.value)}>
            ...
            {members.map(m => (
              <option key={m.member_id} value={m.member_id}>{m.full_name} (ID: {m.member_id})</option>
            ))}
          </select>
      ...
      <table className="w-full bg-white shadow text-center text-sm">
        <thead className="bg-gray-200">
          <tr><th className="p-2">ID</th><th>Book</th><th>Member</th><th>Borrow Date</th><th>Return Date</th><th>Actions</th></tr>
        </thead>
        <tbody>
          {borrows.map(b => (
